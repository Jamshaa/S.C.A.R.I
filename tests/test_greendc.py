...
class TestGreenDCCalculator:
    ...
    @pytest.fixture
    def calculator(self):
        return GreenDCCalculator(electricity_price=0.18, carbon_intensity=0.211, tree_absorption=21.0)
    ...
    def test_operational_impact_calculation(self, calculator):
        result = calculator.calculate_impact(baseline_power_w=100000, scari_power_w=80000, simulation_steps=5000, step_duration_s=1.0)
    ...
    def test_embodied_carbon_calculation_small_dc(self, calculator):
        result = calculator.calculate_embodied_carbon(num_servers=50, topology='spine_leaf')
    ...
    def test_embodied_carbon_calculation_large_dc(self, calculator):
        result = calculator.calculate_embodied_carbon(num_servers=1500, topology='clos')
    ...
    def test_embodied_carbon_calculation_hyperscale(self, calculator):
        result = calculator.calculate_embodied_carbon(num_servers=5000, topology='fat_tree')
    ...
    def test_network_topology_spine_leaf(self, calculator):
        result = calculator.calculate_network_topology(num_servers=500, topology='spine_leaf')
    ...
    def test_network_topology_clos(self, calculator):
        result = calculator.calculate_network_topology(num_servers=300, topology='clos')
        assert result['topology'] == 'clos'
        assert result['oversubscription_ratio'] == 0.67

    def test_network_topology_fat_tree(self, calculator):
        result = calculator.calculate_network_topology(num_servers=400, topology='fat_tree')
        assert result['topology'] == 'fat_tree'
        assert result['total_switches'] > 0

    def test_scenario_comparison(self, calculator):
        result = calculator.compare_scenarios(num_servers=500, baseline_pue=1.67, optimized_pue=1.1, annual_power_kwh=1000000)
    ...
    def test_roi_analysis_positive_return(self, calculator):
        result = calculator.roi_analysis(num_servers=500, investment_eur=500000, annual_savings_eur=100000)
    ...
    def test_roi_analysis_no_investment(self, calculator):
        result = calculator.roi_analysis(num_servers=500, investment_eur=0, annual_savings_eur=100000)
        assert result['investment_eur'] == 0
        assert result['roi_percent_annual'] == 0

    def test_roi_analysis_no_savings(self, calculator):
        result = calculator.roi_analysis(num_servers=500, investment_eur=500000, annual_savings_eur=0)
        assert result['payback_period_years'] is None

    def test_hardware_component(self):
        component = HardwareComponent(name='Test Server', quantity=100, embodied_co2_kg=800, lifespan_years=5.0)
    ...
    def test_impact_with_zero_energy_difference(self, calculator):
        result = calculator.calculate_impact(baseline_power_w=100000, scari_power_w=100000, simulation_steps=5000)
